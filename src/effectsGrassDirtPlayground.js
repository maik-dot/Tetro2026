const canvas = document.getElementById('grassDirtCanvas');
/** @type {CanvasRenderingContext2D | null} */
const ctx = canvas ? canvas.getContext('2d') : null;

const seedInput = document.getElementById('grassDirtSeed');
const grassHInput = document.getElementById('grassDirtGrassH');
const pebblesInput = document.getElementById('grassDirtPebbles');
const redrawBtn = document.getElementById('grassDirtRedraw');

const BLOCK_SIZE = 128;

// Normalisierte Outline eines Gras-Kranzes (0..1 in X/Y)
const GRASS_OUTLINE_NORM = [
  [0.0000, 0.4419],
  [0.0121, 0.4194],
  [0.0241, 0.2957],
  [0.0483, 0.2035],
...
function generateGrassDirtBlock(size = 128, opt = {}) {
  const {
    ...
    borderRadius = 12,
    borderW = 6,
    grassH = 32, // aktuell ungenutzt, Dirt füllt den gesamten Block
    smallStoneCount = 420,
    bigStoneCount = 10,
  } = opt;
  ...
  function rotatePoints(points, cx, cy, ang) {
  ...
  // ---------- Comic-Stein-Generator (analog zu effectsRockPlayground) ----------

  // Eigener deterministischer RNG pro Stein (für Form/Shading)
  function mulberry32Stone(seed) {
    let a = seed >>> 0;
    return function () {
      a |= 0;
      a = (a + 0x6d2b79f5) | 0;
  ...
  function drawDirtStone(ctxLocal, cx, cy, R, seed, opt = {}) {
    const rndStone = mulberry32Stone(seed);
    ...
    const outlineW = opt.outlineW ?? clampStone(R * 0.1, 0.8, 2.0);
    const outlineCol = opt.outlineCol ?? 'rgba(0,0,0,0.55)';

    const baseCol = opt.baseCol ?? 'rgb(150,145,140)';
    const shadowCol = opt.shadowCol ?? 'rgba(0,0,0,0.22)';
    const highlightCol = opt.highlightCol ?? 'rgba(255,255,255,0.18)';
    ...
    const dropAlpha =
      opt.dropShadowAlpha ?? clampStone((R - 1.0) / 6.0, 0.04, 0.24);
    if (dropAlpha > 0.01) {
  ...
  // Inhaltliche "Box" für Erde + Steine sitzt etwas unter dem Gras
  // und etwas schmaler als der gesamte innere Bereich, damit Gras seitlich übersteht.
  const BOX_MARGIN_X = 4;
  ...
  // Für dieses Playground füllt die Erde die gesamte Box.
  const dirtTop = boxY;

  {
    // Leuchtender, orangener Grund wie im Referenzbild
  ...
  // Steine etwas vom Rand der Box abrücken, damit Gras leicht "überhängt".
  const STONE_MARGIN_X = 5;
  ...
  // Hilfsfunktion: zufällige Positionen mit Mindestabstand (einfaches Poisson-ähnliches Sampling)
  function sampleNonOverlappingCenters(count, minDist) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!placed && result.length) {
        // Fallback: wenn kein Platz gefunden wird, kopiere eine existierende Position leicht versetzt
  ...
  for (let i = 0; i < bgCount; i++) {
    ...
    // kleine Steine: Outline nur minimal dunkler als Hintergrund
  ...
  for (let i = 0; i < midCenters.length; i++) {
    ...
    // mittlere Steine: klar dunklere Outline
  ...
  for (let i = 0; i < fgCenters.length; i++) {
    ...
    // große Steine: deutlich dunklere, sehr präsente Outline
...
/*
  // Leichter diagonaler Highlight-Sweep von links oben
  gOv = ctx2.createLinearGradient(ix, iy, ix + iw, iy + ih);
  gOv.addColorStop(0, 'rgba(255,255,255,0.18)');
  gOv.addColorStop(0.35, 'rgba(255,255,255,0.10)');
  gOv.addColorStop(0.7, 'rgba(0,0,0,0.05)');
  gOv.addColorStop(1, 'rgba(0,0,0,0.12)');
  ctx2.fillStyle = gOv;
  ctx2.fillRect(ix, iy, iw, ih);
*/
  ctx2.restore();
  ...
  ctx2.save();
  ctx2.translate(0, 5); // leicht nach unten versetzt
  ctx2.globalAlpha = 1;
...
/*
  ...
  const offsetY2 = grassHeight * 0.12 + 12;
...
function getGrassH() {
  if (!grassHInput) return 34;
  const v = Number(grassHInput.value) || 34;
  return Math.max(20, Math.min(60, v));
}

function getPebbles() {
  if (!pebblesInput) return 140;
  const v = Number(pebblesInput.value) || 140;
  return Math.max(20, Math.min(260, v));
}
...
function drawScene() {
  ...
  const seed = getSeed();
  const grassH = getGrassH();
  const pebbleCount = getPebbles();

  const smallStoneCount = pebbleCount;
  const bigStoneCount = Math.max(4, Math.round(pebbleCount / 25));

  const tile = generateGrassDirtBlock(BLOCK_SIZE, {
    seed,
    grassH,
    smallStoneCount,
    bigStoneCount,
  ...
  // Links: 2×2 Muster
  const scale = 0.31;
  const scaled = BLOCK_SIZE * scale;
  const startX = 24;
  const startY = 32;

  for (let row = 0; row < 2; row++) {
    for (let col = 0; col < 2; col++) {
      const x = startX + col * (scaled + 16);
      const y = startY + row * (scaled + 16);
      ctx.drawImage(tile, x, y, scaled, scaled);
    }
  }

  // Rechts unten: Block 1:1
  const bx = canvas.width - BLOCK_SIZE - 24;
  const by = canvas.height - BLOCK_SIZE - 24;
  ctx.drawImage(tile, bx, by, BLOCK_SIZE, BLOCK_SIZE);
  ...
  ctx.textAlign = 'left';
  ctx.fillText('2×2-Muster (Skalierung 31%)', startX, startY - 10);
  ctx.fillText('Einzelner Block (128×128)', bx, by - 10);
}

if (redrawBtn) redrawBtn.addEventListener('click', drawScene);
if (seedInput) seedInput.addEventListener('change', drawScene);
if (grassHInput) grassHInput.addEventListener('input', drawScene);
if (pebblesInput) pebblesInput.addEventListener('input', drawScene);
